...
| 11 | 喪主に見せる範囲 | **FL担当のタスクは出さない**（2026-08-07変更）。喪主の画面は「ご家族にお願いすること」だけ |
| 12 | 喪主画面の既定 | **未完了のものだけ**を出し、切り替えで完了分を含む全体を見せる |
| 13 | 進捗の分母 | **喪主の担当分のみ** |
| 14 | 支払いの表示 | **告別式の翌日から**（請求書が発行済みなら日付に関わらず表示） |
...
**FL担当のタスクは表示しない**（2026-08-07変更）。当初は「FLが動いている実感」を残すため
状態だけ見せる設計にしていたが、**ご家族から見ると自分がやることと混ざって分かりにくい**ため、
喪主の画面は「ご家族にお願いすること」だけに絞った。FLの進行状況はスタッフから直接伝える。

- サーバー側で `owner = 'fl'` を除いて返す（画面で隠すのではなくAPIが返さない）
- **既定は未完了のものだけ**。「すべて見る」に切り替えるとフェーズ別に完了分も表示する
- チェック＝**「確認しました」**の意思表示。完了の確定はスタッフが行う（§3.3）
- 進捗の分母は**喪主の担当分のみ**。表示している件数と一致する
- お願いすることが無いときは空にせず、「いまお願いすることはありません」と件数を出す
  （空の画面は「見落としているのでは」という不安につながるため）
- 期日を過ぎた未完了は色を変える。並びは期日の近い順

**支払いは告別式の翌日から表示する**。打ち合わせ中の画面に請求の話が出ているのは場面として良くない。

- タスクマスタの `mourner_visible_from`（`always` / `after_ceremony`）で制御し、`/admin` から変更できる
- 支払いフェーズは既定で `after_ceremony`
- 判定は見積の葬儀日から自動。**請求書が発行済みなら日付に関わらず表示**する
- 条件を満たすまでは、タスクだけでなく**請求金額もAPIが返さない**
- 表示後は、総額・支払期限・お振込先を出し、明細は「内訳を見る」で開く
...
5. migrations/024_task_mourner_visibility.sql     を実行（支払いを葬儀後に出す）
6. npx supabase functions deploy task-public
7. npm run build:tasks  → dist-tasks/ を配信（Vercel の別プロジェクト）
8. /admin →「タスクマスタ管理」で喪主サイトのURLを設定
```
