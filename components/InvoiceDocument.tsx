            ...
            <div className="mb-6" style={{ marginTop: '20px' }}>
                <div className="border border-black text-sm">
                    ...
                    <div className="flex bg-gray-100 font-bold border-b border-black !print-color-adjust-exact">
                        <div className="w-3/5 text-center py-1 px-4 border-r border-black tracking-widest">プラン名</div>
                        <div className="w-2/5 text-center py-1 px-4 tracking-widest">金額 (税抜)</div>
                    </div>
                    ...
                    <div>
                        <div className={`flex ${includedItems.length === 0 && taxableOptions.length === 0 && nonTaxableOptions.length === 0 ? '' : 'border-b border-black'}`}>
                            <div className="w-3/5 text-left py-2 px-2 border-r border-black font-medium">{`基本プラン (${plan.name})`}</div>
                            <div className="w-2/5 text-right py-2 px-2 font-mono">¥{plan.price.toLocaleString()}</div>
                        </div>
                        {includedItems.length > 0 && includedItems.map((item, i) => (
                            <div key={`inc-${item.id}`} className={`flex ${i === includedItems.length - 1 && taxableOptions.length === 0 && nonTaxableOptions.length === 0 ? '' : 'border-b border-black'}`}>
                                <div className="w-3/5 text-left py-2 px-2 border-r border-black text-gray-700 flex items-center">
                                    <span className="w-1.5 h-1.5 rounded-full bg-gray-400 mr-2 inline-block"></span>
                                    {item.name}
                                </div>
                                <div className="w-2/5 text-right py-2 px-2 font-mono text-gray-500">プラン内</div>
                            </div>
                        ))}
                    </div>
                    ...
                    {taxableOptions.length > 0 && (
                        <>
                            <div className="flex bg-gray-100 font-bold border-b border-black !print-color-adjust-exact">
                                <div className="w-2/5 text-center py-1 px-2 border-r border-black tracking-widest">オプション内訳 / 項目名</div>
                                <div className="w-1/5 text-center py-1 px-2 border-r border-black tracking-widest">詳細</div>
                                <div className="w-2/5 text-center py-1 px-2 tracking-widest">金額 (税抜)</div>
                            </div>
                            <div>
                                {taxableOptions.map((item, i) => {
                                    return (
                                        <div key={`opt-${item.id}`} className={`flex ${i === taxableOptions.length - 1 && nonTaxableOptions.length === 0 ? '' : 'border-b border-black'}`}>
                                            <div className="w-2/5 text-left py-2 px-2 border-r border-black flex items-center truncate">
                                                {item.name}
                                            </div>
                                            <div className="w-1/5 text-center py-2 px-2 border-r border-black text-gray-600 truncate">
                                                {getGradeLabel(item)}
                                            </div>
                                            <div className="w-2/5 text-right py-2 px-2 font-mono">¥{getPrice(item).toLocaleString()}</div>
                                        </div>
                                    );
                                })}
                            </div>
                    ...
                    {nonTaxableOptions.length > 0 && (
                        <>
                            <div className="flex bg-gray-100 font-bold border-b border-black !print-color-adjust-exact">
                                <div className="w-2/5 text-center py-1 px-2 border-r border-black tracking-widest">非課税対象</div>
                                <div className="w-1/5 text-center py-1 px-2 border-r border-black"></div>
                                <div className="w-2/5 text-center py-1 px-2 tracking-widest">金額</div>
                            </div>
                            <div>
