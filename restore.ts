...
const recoverContent = fs.readFileSync('recover_data.json', 'utf8');
const recover = JSON.parse(recoverContent).items;

const toPgArray = (arr: any) => arr && arr.length > 0 ? `ARRAY['${arr.join("','")}']` : `ARRAY[]::text[]`;
...
for (let rItem of recover) {
  ...
  if (rItem.options) {
      if (cItem && cItem.options) {
          rItem.options.forEach((ropt: any) => {
